...
moment.tz.setDefault("Asia/Kolkata");
  ...
  const getLastBrokenStreak = (activationDates) => {
    ...
    console.log("here is the data ===>", moment().diff(lastCaseActivatedDay, 'day'));
  ...
  const calculateUniqueStreak = (activationDates) => {
    ...
    if (moment(todayDate).isSame(lastCaseActivatedDay, 'day') && currentStreak == 0) {
      currentStreak = 1
    }
...
app.get('/get-client-streak' , async (req, res) => {
    ...
    const response = await axios.get('https://answermagic.5cn.co.in/api/v1/db/data/v1/p7ta4753b4qod00/ScratchCardLogs?limit=200000', {
    ...
    for (const [clientId, data] of Object.entries(clientMap)) {
      if(clientId == '1549') {
        ...
        const obj = {
          streak : calculateUniqueStreak(data?.map((elem) => moment(elem.created_at))),
          lastBrokenStreak : getLastBrokenStreak(data?.map((elem) => moment(elem.created_at)))?.toLocaleString(),
          isStreakBroken: isStreakBroken(data?.map((elem) => moment(elem.created_at))),
        }
